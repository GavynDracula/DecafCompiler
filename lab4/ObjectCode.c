...
int GetReg(FILE* fp, Scope variable)
{
    ...
    if (variable.type == ScopeFormal)
    {
        ...
        {
            // The vairalble has been allocated register
    ...
    // If the variable is local vairalble
    else if (variable.type == ScopeLocal)
    {
        ...
        else
        {
            ...
            for (i = 8; i <= 25; i++)
            {
                if (RegDepictor[i].used == 0)
                {
                    // Find the least recently used register(LRU)
...
void GenerateObjectCode(FILE* fp, TACCode* entrance)
{
    ...
    while (tmp)
    {
        ...
        if (darg == 1)
            fprintf(fp, "######   Correspond to source file line %3d   ######\n", tmp->line);
        tmp = tmp->next;
        line += 1;
    }
    if (mulop != 0)
    {
        ...
        fprintf(fp, "jr   $ra\n");
        if (darg == 1)
            fprintf(fp, "######   Use booth multipy to replace instrunction \"mul\"   ######\n");
    }
}
